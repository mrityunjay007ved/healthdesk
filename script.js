...
window.showLoginModal = function(userType) {
	...
	if (userType === 'member') {
		...
		document.querySelector('.submit-btn').style.background = 'linear-gradient(135deg, #667eea, #764ba2)';
		...
		document.querySelector('.submit-btn').style.background = 'linear-gradient(135deg, #e74c3c, #f39c12)';
		const cf = document.getElementById('doctorConfirmPasswordField');
		if (cf) cf.style.display = 'none';
		isDoctorSignup = false;
		const mcf = document.getElementById('memberConfirmPasswordField');
